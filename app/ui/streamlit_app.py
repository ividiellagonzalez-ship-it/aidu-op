...
def _force_schema_v18():
    """
    Fuerza la creación de tablas v18 SIN depender del cache de Streamlit.
    
    Crítico en producción: si la BD se restaura desde el repo después de que
    el cache se llenó, las tablas v18 NO se crean. Esta función corre en cada
    arranque y es 100% idempotente (CREATE TABLE IF NOT EXISTS).
    """
    try:
        from app.db.migrator import _auto_reparar_schema, get_connection
        conn = get_connection()
        try:
        ...
        print(f"⚠️ Error en _force_schema_v18: {e}")
...
_force_schema_v18()  # SIEMPRE corre, sin cache → garantiza tablas v18
